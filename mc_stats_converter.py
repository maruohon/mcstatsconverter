...
def print_help():
    print "Usage: " + sys.argv[0] + " [--pretty] [--mappings=path/to/mappingsfile] <statsdir>"
    print ""
...
if len(sys.argv) > 1:
    ...
    for s in sys.argv[1:len(sys.argv)]:
        if s.startswith('-'):
            ...
            print "Unrecognized/extraneous parameter '" + s + "'"
            exit(1)
else:
    print_help()
    exit(1)
...
if os.path.isfile(mappingsfilename) == False:
    print "Error: Could not find the mappings file '" + mappingsfilename + "'"
    exit(1)
...
with open(mappingsfilename, 'r') as mappingsfile:
    ...
    for line in lines:
        ...
        if match != None:
            ...
            # print "match: " + keyid + " => " + keystrmod + " (" + keystr + ")"
    ...
    for oldkey in oldstats:
        ...
        if startpos != 0:
            ...
            if not oldid in keymappings:
                print "Error: missing mapping for id '" + oldid + "'"
                exit(1)

            newkey = oldkey[0:startpos] + keymappings[oldid]
            # print "mapping key: " + oldkey + " => " + newkey
